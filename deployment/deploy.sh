...
docker run --rm -v $(pwd):/srv/jekyll -v $(pwd)/vendor/bundle:/usr/local/bundle -e "TZ=Europe/Berlin" -e "LANG=C.UTF-8" jekyll/jekyll:4.2.0 jekyll build --config $SITE_CONFIG --no-watch
